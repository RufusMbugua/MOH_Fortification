 ...
 */
use application\models\Entities\entities_sugar\E_Sugar_InternalFort_C3;

class M_Sugar_InternalFort_C3  extends MY_Model {
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$manufacturer=$this->em->getRepository('models\Entities\E_ManufacturerCompound')
			 ...
			 $this -> theForm = new \models\Entities\entities_sugar\E_Sugar_InternalFort_C3(); //create an object of the model
		      
			 	
				$this -> theForm -> setDates($this->elements[$i]['dates']);
				$this -> theForm -> setLotID($this->elements[$i]['lotID']);
				$this -> theForm -> setBagsProduced($this->elements[$i]['bagsProduced']);
				$this -> theForm -> setDeliveryNumber($this->elements[$i]['deliveryNumber']);
				$this -> theForm -> setFactoryName($this->elements[$i]['factoryName']);
				...
				$this -> theForm -> setTransactedBy($this->input->post('nameResponsible'));
				$this -> theForm -> setTransactionDate($this->input->post('dateResponsible'));
